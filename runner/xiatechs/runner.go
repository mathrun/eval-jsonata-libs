...
func (r *XiaTechsRunner) getData(tc *TestCase) (interface{}, error) {
	if tc.Dataset != "" {
...
func (r *XiaTechsRunner) Eval(expr string, data interface{}, bindings map[string]interface{}) (result interface{}, err error) {
	defer func() {
...
func (r *XiaTechsRunner) RunTestCase(testCase *TestCase) *TestCaseResult {
	// xiatechs has no tail-call optimisation; recursive expressions overflow the Go stack
	// which is a fatal crash that cannot be recovered, so skip that group entirely.
	if testCase.Category == "tail-recursion" {
		return &TestCaseResult{TestCase: testCase, Passed: false, Error: fmt.Errorf("skipped: tail-recursion not supported by xiatechs")}
	}
	...
	// xiatechs uses different error codes than the JSONata spec, so only check that an error occurred.
...
func (r *XiaTechsRunner) RunTests(testData *TestData) (*RunnerResult, error) {
